...
class HeaderMenuRepo
{
    ...
    public function getMenus(): array
    {
        ...
        // Group categories under "Product Catalogue" dropdown
        $productCatalogue = $this->createProductCatalogueMenu($categories);
        
        $menus = array_merge($specials, [$productCatalogue], $catalogs, $pages);

        return fire_hook_filter('global.header.menus', $menus);
    }

    /**
     * Create Product Catalogue menu with categories as children
     *
     * @param array $categories
     * @return array
     */
    private function createProductCatalogueMenu(array $categories): array
    {
        if (empty($categories)) {
            return [];
        }
        ...
        return [
            'name' => trans('front/common.product_catalogue'),
            'url' => '#',
            'children' => $categories,
        ];
    }
